...
sudo kubeadm init --pod-network-cidr=10.244.0.0/16 --cri-socket /run/containerd/containerd.sock --apiserver-advertise-address 192.168.57.8 
...
echo "kubeadm join $master_ip:6443 --token=$token --discovery-token-ca-cert-hash sha256:$sha_token" > join_command.txt
